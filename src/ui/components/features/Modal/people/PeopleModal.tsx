...
import { Modal, ModalContent, ModalHeader, ModalBody, Spinner, Card, CardBody, Chip } from "@nextui-org/react";
import { getFilmsList } from '@/api/services/film.service';
import AxiosInstance from '@/api/AxiosInstance';
import { Calendar, Eye, Palette, Ruler, UserCircle, Weight } from 'lucide-react';
        ...
        const fetchStarshipsNames = async () => {
            if (character?.starships && character.starships.length > 0) {
                ...
                try {
                    ...
                    const starshipsPromises = character.starships.map((starshipsUrl: any) =>
                        AxiosInstance('get', starshipsUrl)
                    );

                    // Wait for all requests to complete
                    const starshipsData = await Promise.all(starshipsPromises);
                    console.log("🚀 ~ fetchStarshipsNames ~ starshipsData:", starshipsData)

                    // Extract just the titles from the starships objects
                    const starshipsTitles = starshipsData.map(film => film.name);
                    console.log("🚀 ~ fetchStarshipsNames ~ starshipsTitles:", starshipsTitles)
